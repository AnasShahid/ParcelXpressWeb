...
namespace ParcelXpress.Enums
{
    ...
    public class StringEnum
    {
        ...
        public static string GetDisplayName(Enum value)
        {
            ...
            if (_displayNames.ContainsKey(value))
                output = (_displayNames[value] as DisplayNameAttribute).Value;
            else
            {
                //Look for our 'StringValueAttribute' in the field's custom attributes
                FieldInfo fi = type.GetField(value.ToString());
                DisplayNameAttribute[] attrs = fi.GetCustomAttributes(typeof(DisplayNameAttribute), false) as DisplayNameAttribute[];
                if (attrs.Length > 0)
                {
                    _displayNames.Add(value, attrs[0]);
                    output = attrs[0].Value;
                }
    ...
    #region Display Attribute Class
